...
function initall() {
	...
	var setmorePopup    =    function(k,isReschedule,isbookAppointmentResource,e)
	{
		...
		if( windowWidth < 600 )
		{
			window.open( filePath+'bookingpage/'+k , '_blank' );
			return;
		}
		...
		loadIframe		=	function()
							{
								if(isReschedule)
								{
									if(isbookAppointmentResource=="true")
									{
										...
										jQuery("#iframeContent").html('<iframe id="setmore-fancy-box-iframe" onload="return initalFrameLoad();" frameborder="0" hspace="0" scrolling="auto" src="'+filePath+'rescheduleAppointment.do?cancellationKey='+k+'&isStaffBookingPage=true"></iframe>');
									}
								else
									{
										jQuery(".iframe_loader").show();
										jQuery("#iframeContent").hide();
										jQuery("#iframeContent").html('<iframe id="setmore-fancy-box-iframe" onload="return initalFrameLoad();" frameborder="0" hspace="0" scrolling="auto" src="'+filePath+'rescheduleAppointment.do?cancellationKey='+k+'"></iframe>');
									}
								}
								else
								{
									jQuery(".iframe_loader").show();
									jQuery("#iframeContent").hide();
									jQuery("#iframeContent").html('<iframe id="setmore-fancy-box-iframe" onload="return initalFrameLoad();" frameborder="0" hspace="0" scrolling="auto" src="'+filePath+'bookingpage/'+k+'"></iframe>');
								}
							};
		bindEvents		=	function()
							{
